import threading
import time

import cv2
import numpy as np
from android_tool import AndroidTool
from argparses import args
from getReword import GetRewordUtil
from globalInfo import GlobalInfo

from wzry_env import Environment
from onnxRunner import OnnxRunner
from ppoAgent import PPOAgent

# 全局状态
globalInfo = GlobalInfo()

class_names = ['started']
start_check = OnnxRunner('models/start.onnx', classes=class_names)

rewordUtil = GetRewordUtil()
tool = AndroidTool()
tool.show_scrcpy()
# tool.show_action_log()
env = Environment(tool, rewordUtil)

# 使用PPO Agent替代DQN Agent
agent = PPOAgent()

# PPO训练需要存储一个完整轨迹
rollout_buffer = []

def data_collector():
    global rollout_buffer
    
    while True:
        # 获取当前的图像
        state = tool.screenshot_window()
        # 保证图像能正常获取
        if state is None:
            time.sleep(0.01)
            continue
        # cv2.imwrite('output_image.jpg', state)
        # 初始化对局状态 对局未开始
        globalInfo.set_game_end()
        # 判断对局是否开始
        checkGameStart = start_check.get_max_label(state)

        if checkGameStart == 'started':
            print("-------------------------------对局开始-----------------------------------")
            globalInfo.set_game_start()
            
            # 重置轨迹缓冲区
            rollout_buffer = []
            hidden_state = None
            prev_value = 0.0

            # 对局开始了，进行训练
            while globalInfo.is_start_game():
                # 获取预测动作
                action, log_prob, value, hidden_state = agent.select_action(state, hidden_state)

                next_state, reward, done, info = env.step(action)
                print(info, reward)

                # 存储轨迹数据
                rollout_buffer.append({
                    'state': state,
                    'action': action,
                    'log_prob': log_prob,
                    'reward': reward,
                    'value': value,
                    'next_value': prev_value,  # 下一状态的值（延迟一步）
                    'done': done
                })
                prev_value = value

                # 对局结束
                if done == 1:
                    print("-------------------------------对局结束-----------------------------------")
                    globalInfo.set_game_end()
                    
                    # 处理最后一个样本的next_value
                    if rollout_buffer:
                        rollout_buffer[-1]['next_value'] = 0.0  # 终止状态的值为0
                    
                    # 将轨迹数据存入全局记忆
                    for item in rollout_buffer:
                        globalInfo.store_transition_ppo(
                            item['state'], item['action'], item['log_prob'],
                            item['reward'], item['value'], item['next_value'], item['done']
                        )
                    
                    break

                state = next_state

        else:
            print("对局未开始")
            time.sleep(0.1)


def train_agent():
    count = 1
    while True:
        if not globalInfo.is_memory_bigger_batch_size_ppo():
            time.sleep(1)
            continue
        print("training")
        
        # 获取PPO批次数据
        transitions = globalInfo.random_batch_size_memory_ppo()
        
        # 提取数据
        states = [t.state for t in transitions]
        actions = [t.action for t in transitions]
        log_probs = [t.log_prob for t in transitions]
        rewards = [t.reward for t in transitions]
        values = [t.value for t in transitions]
        next_values = [t.next_value for t in transitions]
        dones = [t.done for t in transitions]
        
        # 训练PPO
        agent.train(states, actions, log_probs, rewards, values, next_values, dones)
        
        if count % args.num_episodes == 0:
            agent.save_model('src/wzry_ai_ppo.pt')
        count = count + 1
        if count >= 100000:
            count = 1


if __name__ == '__main__':
    training_thread = threading.Thread(target=train_agent)
    training_thread.start()
    data_collector()